...
@dataclass
class SessionInfo:
    """Single authority for all data in session_info.json.

    Core fields (last_modified_date, created_date, name) are stored as dataclass
    attributes. Additional sections (e.g. hand_eye_configuration) are kept in
    _extra_data and persisted alongside the core fields. All writes to
    session_info.json should go through this class to prevent data loss.
    """
    ...
    def save(self) -> None:
        json_path = self.root_folder / self.name / "session_info.json"
    ...
    def from_existing(cls, root_folder: Path, session_name: str) -> "SessionInfo":
        with open(root_folder / session_name / "session_info.json", "r", encoding="utf-8") as session_file:
            ...
            instance._extra_data = {k: v for k, v in session_data.items() if k not in core_keys}
            return instance
