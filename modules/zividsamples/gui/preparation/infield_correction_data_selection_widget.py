    ...
    def run(self):
        for file in self._files:
            if self._cancel_event.is_set():
                break
            try:
                data = InfieldCorrectionInputDataCore.from_path(file, self._cv2_handler)
                self.item_loaded.emit(self._generation, data)
...
class InfieldCorrectionDataSelectionWidget(QWidget):
    ...
    def __init__(self, directory: Path, parent=None):
        ...
        self._loader_worker: Optional[_InfieldLoadWorker] = None
