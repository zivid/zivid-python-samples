...
async def do_move_to_place(robot, place_pose, out_of_fov_pose):

    # Move to place location
    print("start move",flush=True)
    ...
    print("start move 2",flush=True)

    # Move back to "out of FOV" location
    robot.MoveJ(out_of_fov_pose)
    #robot.WaitMove()
    #time.sleep(0.2)
    print("finish move",flush=True)

async def do_capture(camera, settings):
    print("start capt",flush=True)
    frame = camera.capture(settings)
    print("finish capt",flush=True)
    #return frame
...
def _main() -> None:
    ...
    settings = zivid.Settings.load("Settings01.yml")
    ...
    robot_speed_accel_limits = [100, 100, 100, 100]
    ...
    #t1 = multiprocessing.Process(target=do_move_to_place(robot, targets[3], targets[2]))
    #t2 = multiprocessing.Process(target=do_capture(camera, settings))

    # Move to place location
    print("start move",flush=True)
    robot.MoveJ(targets[3])
    #robot.WaitMove()
    ...
    # Move back to "out of FOV" location
    robot.MoveJ(targets[2], blocking = False)
    ...
    #task_1 = asyncio.create_task(do_capture(camera, settings))
    #task_2 = asyncio.create_task(do_move_to_place(robot, targets[3], targets[2]))
